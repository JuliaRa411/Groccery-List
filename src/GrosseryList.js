...
GrosseryList:[]
    ...
    addItem(input){
        ...
        let listArray = this.state.GrosseryList;
        listArray.push(input);
        this.setState({GrosseryList: listArray, userInput: ""})
...
deleteItem(input){
    let listArray = this.state.GrosseryList;
    listArray =[];
    this.setState({GrosseryList: listArray})
}
    crossedWord(event){
...
e.preventDefault()
    ...
    render(){
        return(
            ...
            <ul>
                {this.state.GrosseryList.map((item, index) =>(
